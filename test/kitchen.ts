...
const execOpts: Pick<cp.SpawnSyncOptionsWithStringEncoding, "cwd" | "encoding"> = {
...
describe("🚰 kitchen sink", () => {
  ...
  it("should use as a non-locally installed module", () => {
    ...
    fs.copySync(path.join(stagingPath, "gts.tgz"), path.join(tmpDir.name, "gts.tgz"));
    // It's important to use `-n` here because we don't want to overwrite
    // the version of gts installed, as it will trigger the npm install.
    ...
    assert.deepStrictEqual(tsconfig.extends, "./node_modules/gts/tsconfig-google.json");
  ...
  it("should terminate generated files with newline", () => {
    ...
    assert.ok(fs.readFileSync(path.join(kitchenPath, "package.json"), "utf8").endsWith("\n"));
  ...
  it("should lint before fix", async () => {
    // @ts-ignore
    const res = await execa("npm", ["run", "lint"], Object.assign({}, { reject: false }, execOpts));
